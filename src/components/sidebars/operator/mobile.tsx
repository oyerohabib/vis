...
const MobileOperatorSidebar = () => {
  const { setOpenOperatorSidebar, openOperatorSidebar } = useStateCtx();
    ...
    <>
      <div
        className={cn(
          ...
          openOperatorSidebar ? "opacity-100" : "opacity-0 pointer-events-none "
        )}
        onClick={() => setOpenOperatorSidebar(false)}
      />
      <section
        className={cn(
        ...
        <ul className="flex flex-col gap-y-3 min-[400px]:gap-y-4 py-8 mt-6">
          {OPERATOR_SIDEBAR_LINKS.map((link) => (
            <Link
              ...
              onClick={() => {
                setActiveLink(`/operator/${link.link}`);
                setOpenOperatorSidebar(false);
              }}
            >
              <link.icon
